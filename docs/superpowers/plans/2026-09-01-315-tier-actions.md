...
- **Response shape:** every API error is `{"error": "<message>"}` and, where a client must branch on it, also `"code": "<snake_case>"`. Codes used here: `actions_disabled`, `impersonation_forbidden`, `not_connected`, `unknown_target`, `invalid_kind`, `invalid_targets`, `nothing_to_undo`, `nothing_to_retry`, `batch_running`, `not_found`, `db_error`.
...
| POST | `/api/actions/batches/{id}/retry` | 202 `{batch_id}` (a NEW batch over the failed rows, spec §8) | 400 `nothing_to_retry`, 404, 409 (`not_connected`, or `batch_running` while the batch is still queued/running), 403, 503 |
