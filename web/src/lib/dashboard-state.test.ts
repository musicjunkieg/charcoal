...
describe('dashboardView', () => {
	...
	it('shows results whenever any accounts are scored, scanning or not', () => {
		const counts = { high: 1, elevated: 0, watch: 2, low: 5, not_assessed: 0, expired: 0, total: 8 };
	...
	it('shows results with data even if started_at is missing (server restarted)', () => {
		...
		const counts = { high: 0, elevated: 0, watch: 0, low: 3, not_assessed: 0, expired: 0, total: 3 };
	...
	it('shows results (not all-clear) when a completed scan scored only not_assessed accounts (#222)', () => {
		...
		const counts = { high: 0, elevated: 0, watch: 0, low: 0, not_assessed: 4, expired: 0, total: 0 };
		expect(dashboardView(status({ tier_counts: counts, started_at: '2026-07-05T12:00:00Z' }))).toBe(
			'results'
		);
	});
